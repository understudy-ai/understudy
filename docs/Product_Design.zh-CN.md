...
> **Understudy 是一个可教学的桌面 Agent。** 它像人类同事一样操作你的电脑 —— GUI、浏览器、Shell、文件系统 —— 从演示中学习，把成功路径固化为可复用技能，并持续优化执行效率。
...
**跨次反馈机制：** 运行时在内存中维护 `groundingFeedback` Map，按 `[app, scope, action, target]` 为 key，保留最近 2 分钟内最多 2 条失败记录。下次同一目标的 grounding 请求会携带这些失败信息，让模型避免重复错误。成功后清除对应 key 的记录。
...
3. **GUI 回放提示**（`## Detailed GUI Replay Hints`）— 仅作为兜底。失败策略要求：执行前 `gui_read` 确认目标可见；目标描述从当前截图重新获取，不用录制时的坐标；路线偏离时重新规划而非盲目回放
...
Route guard（`route-guard-policy`）追踪每个路线类别的连续失败次数。当前追踪的路线：`gui`、`browser`、`web`、`shell`、`process`。
...
- **屏幕录制权限** — 截图类工具（grounding、read、screenshot）必需
...
实际运行中，Layer 3 crystallized 的 skill 起初继承原始工作中观察到的路线。随时间推移，Layer 4 的机制（route guard、偏好引导、teach 标注）推动同一个 skill 向更快的执行路径演进。
...
- 13 个 GUI 工具 + grounding（30/30 基准测试）
...
- Layer 4 路线发现 — 路线偏好、route guard、teach 路线标注和浏览器自动降级已实现；主动路线提升和自动路线发现是后续工作
