...
from story_context import load_story_notes, select_story_visuals
...
def sanitize_narration_lines(lines: list[str], full_name: str, alias: str) -> list[str]:
    ...
    for line in lines:
        ...
        # Narration lines should stay speakable and compact; do not over-truncate.
    ...
    # Only compress if total exceeds the compact review budget.
    if total_words(sanitized) > 320:
...
def compress_narration_lines(lines: list[str], *, max_lines: int, max_words: int) -> list[str]:
    cleaned = [clean_text(line) for line in lines if clean_text(line)]
    if not cleaned:
        return []
    total_words = sum(len(line.split()) for line in cleaned)
    if len(cleaned) <= max_lines and total_words <= max_words:
        return cleaned

    keep_indices: set[int] = {0, len(cleaned) - 1}
    if len(cleaned) > 1:
        keep_indices.add(1)
    if len(cleaned) > 3:
        keep_indices.add(len(cleaned) - 2)

    remaining_slots = max(0, max_lines - len(keep_indices))
    middle_candidates = list(range(2, max(2, len(cleaned) - 2)))
    if remaining_slots > 0 and middle_candidates:
        if len(middle_candidates) <= remaining_slots:
            keep_indices.update(middle_candidates)
        else:
            for slot in range(remaining_slots):
                pick = round((slot + 0.5) * len(middle_candidates) / remaining_slots - 0.5)
                keep_indices.add(middle_candidates[max(0, min(len(middle_candidates) - 1, pick))])

    selected = [cleaned[index] for index in sorted(keep_indices)]
    while len(selected) > max_lines:
        selected.pop(-2 if len(selected) > 2 else -1)
    while sum(len(line.split()) for line in selected) > max_words and len(selected) > 5:
        middle_indices = list(range(1, len(selected) - 1))
        if not middle_indices:
            break
        drop_index = max(middle_indices, key=lambda index: len(selected[index].split()))
...
def feedback_signal_text(feedback: dict[str, Any]) -> str:
    parts: list[str] = [
        clean_text(feedback.get("summary")),
        clean_text(feedback.get("publishReadiness")),
        clean_text(feedback.get("revisionMode")),
    ]
    visual_risks = as_dict(feedback.get("visualRisks"))
    parts.extend(
        clean_text(visual_risks.get(key))
        for key in [
            "browserOrDesktopIntrusion",
            "textDensity",
            "productFrameDominance",
            "packagingOverhang",
        ]
    )
    for item in feedback.get("topIssues", [])[:4]:
        issue = as_dict(item)
        parts.extend(
            [
                clean_text(issue.get("issue")),
                clean_text(issue.get("whyItMatters")),
                clean_text(issue.get("suggestedFix")),
            ]
        )
    parts.extend(as_list(feedback.get("onePassFixPlan"))[:4])
    return " ".join(part.lower() for part in parts if part)


def feedback_mentions(feedback: dict[str, Any], *terms: str) -> bool:
    signal = feedback_signal_text(feedback)
    return any(term.lower() in signal for term in terms)
...
def main(root_dir: str) -> int:
    ...
    visual_selection = select_story_visuals(root, notes)
    ...
    media_clips = [
        clip
        for clip in as_list(as_dict(notes.get("media")).get("clips"))
    ...
    def media_clip(index: int) -> str:
        return media_clips[index] if index < len(media_clips) else ""

    first_screen_rel = first_non_empty(
        clean_text(visual_selection.get("firstImpression")),
        rel_if_exists(root / "experience" / "screenshots" / "01-First-Screen.png"),
        media_shot(0),
    )
    main_screen_rel = first_non_empty(
        clean_text(visual_selection.get("coreTaskScreen")),
    ...
    store_detail_rel = first_non_empty(
        clean_text(visual_selection.get("context")),
        rel_if_exists(root / "topic" / "screenshots" / "02-iPhone-App-Store-Detail.png"),
    )
    core_clip_rel = rel_if_exists(core_clip)
    secondary_clip_rel = rel_if_exists(secondary_clip)

    secondary_asset_rel = first_non_empty(
        clean_text(visual_selection.get("secondaryProof")),
        secondary_clip_rel,
        secondary_feature_rel,
        pricing_limit_rel,
    )
    browser_cleanup_requested = feedback_mentions(
        feedback,
        "browser",
        "desktop",
        "app store surface",
    ...
    packaging_cleanup_requested = feedback_mentions(
        feedback,
        "packaging",
        "title card",
        "verdict card",
    ...
    repetition_cleanup_requested = feedback_mentions(
        feedback,
        "reused",
        "repeat",
        "repetitive",
    ...
    proof_first_edit = (
        clean_text(feedback.get("revisionMode")) == "edit"
        or clean_text(feedback.get("publishReadiness")) == "needs_revision"
        or browser_cleanup_requested
        or packaging_cleanup_requested
        or repetition_cleanup_requested
    )

    secondary_exists = bool(secondary_asset_rel)
    opening_asset_rel = first_non_empty(
        clean_text(visual_selection.get("opening")),
        core_clip_rel,
        media_clip(0),
        first_screen_rel,
        main_screen_rel,
    ...
    first_impression_asset_rel = first_non_empty(
        clean_text(visual_selection.get("firstImpression")),
        first_screen_rel,
        main_screen_rel if main_screen_rel != opening_asset_rel else "",
        core_task_rel if core_task_rel != opening_asset_rel else "",
        outcome_rel if outcome_rel != opening_asset_rel else "",
    )
    core_source = first_non_empty(
        clean_text(visual_selection.get("coreTask")),
        core_clip_rel,
        media_clip(0),
        core_task_rel,
        main_screen_rel,
    ...
    distinct_first_impression = (
        first_impression_asset_rel
        and first_impression_asset_rel not in {opening_asset_rel, core_source}
    )
    first_impression_exists = bool(
        distinct_first_impression
        and not (proof_first_edit and core_clip_exists)
    )
    context_beat = (
        not proof_first_edit
        and include_context_beat(video_plan_notes, feedback)
        and bool(store_detail_rel)
        and demo_depth != "shallow"
        and not core_clip_exists
    )
    ...
    runtime_target = (
        "about 95-130 seconds"
        if proof_first_edit and core_clip_exists
        else "about 130-160 seconds"
    ...
    if len(narration_lines) < 3:
        narration_lines = fallback_narration
    if proof_first_edit:
        narration_lines = compress_narration_lines(
            narration_lines,
            max_lines=10,
            max_words=220,
        )
    ...
    outcome_source = first_non_empty(
        clean_text(visual_selection.get("outcome")),
        outcome_rel,
        secondary_feature_rel if secondary_feature_rel != core_source else "",
        pricing_limit_rel if pricing_limit_rel != core_source else "",
        core_source,
        opening_asset_rel,
    )
    add_beat(
    ...
    plan_lines = [
        ...
        story_angle,
        "",
        "# Default Local Edit Route",
        "- Default happy path: render proof panels locally, assemble the timeline with the repo ffmpeg helpers, and verify `post/final-video.mp4` with `ffprobe`.",
        "- Keep the whole route local: no GUI editor is required for the default render.",
        f"- Stable render identity: `{project_name}` / `{run_slug}`.",
        "",
        "# Shot Priority",
        ...
        "- Prefer concise overlays and proof-led cards over dense text panels.",
        ...
        "- Use the prepared proof assets described in `post/video-edit-manifest.json`, not the raw Stage 2 screenshots directly.",
    ...
    shot_list_lines = [
        "# Video Shot List",
        "- Follow this beat order directly in the local render so the product stays dominant and the packaging stays brief.",
    ...
    import_manifest = {
        ...
        "proofFirstEdit": proof_first_edit,
        ...
        "visualSelection": {
            key: value
            for key, value in visual_selection.items()
            if key != "candidates"
        },
        "visualCandidates": [
            {
            ...
            for candidate in visual_selection.get("candidates", [])
        ],
        "beats": [
            {
                "order": index + 1,
                ...
                "reusePrevious": beat.get("reusePrevious") or "false",
            }
            for index, beat in enumerate(beat_specs)
        ],
        "importAssets": [
    ...
    (post_dir / "video-shot-list.md").write_text("\n".join(shot_list_lines).rstrip() + "\n")
    (post_dir / "video-edit-manifest.json").write_text(json.dumps(import_manifest, indent=2, ensure_ascii=False) + "\n")
        ...
        json.dumps(
            {
                ...
                "shotList": str(post_dir / "video-shot-list.md"),
                "importManifest": str(post_dir / "video-edit-manifest.json"),
                "alias": alias,
                "slides": slide_order,
                "clips": [
