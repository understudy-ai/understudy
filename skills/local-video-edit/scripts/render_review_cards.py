...
from story_context import select_story_visuals
...
def choose_story_screens(root: Path, selection: dict[str, object]) -> tuple[Path, Path, Path | None]:
    ...
    def selected_path(key: str) -> Path | None:
        rel = clean_text(str(selection.get(key) or ""))
        candidate = root / rel if rel else None
        if candidate and candidate.exists() and candidate.suffix.lower() in {".png", ".jpg", ".jpeg", ".webp"}:
            return candidate
        return None

    core = first_existing(
        [
            selected_path("coreTaskScreen"),
            core_screen,
            main_screen,
            first_screen,
        ]
    ...
    outcome = first_existing(
        [
            selected_path("outcomeScreen"),
            outcome_screen if outcome_screen != core else None,
            secondary_screen if secondary_screen != core else None,
            limit_screen if limit_screen != core else None,
            main_screen if main_screen != core else None,
            first_screen if first_screen != core else None,
        ]
    )
    if outcome is None:
        outcome = core

    secondary = first_existing(
        [
            selected_path("secondaryProofScreen"),
            secondary_screen if secondary_screen not in {core, outcome} else None,
            limit_screen if limit_screen not in {core, outcome} else None,
            selected_path("firstImpression") if selected_path("firstImpression") not in {core, outcome} else None,
            main_screen if main_screen not in {core, outcome} else None,
        ]
    )
    if secondary and image_difference_score(secondary, core) < 0.04 and image_difference_score(secondary, outcome) < 0.04:
        secondary = None
...
def render_title_card(
    ...
    *,
    ...
    shot = hero_shot if hero_shot and hero_shot.exists() else root / "experience" / "screenshots" / "02-Main-Screen.png"
    if not shot.exists():
...
def main(root_dir: str) -> None:
    ...
    selection = select_story_visuals(root, notes)
    ...
    core_shot, outcome_shot, secondary_shot = choose_story_screens(root, selection)
    first_shot = root / clean_text(str(selection.get("firstImpression") or ""))
    if not first_shot.exists() or first_shot.suffix.lower() not in {".png", ".jpg", ".jpeg", ".webp"}:
        first_shot = root / "experience" / "screenshots" / "01-First-Screen.png"
    store_shot = root / clean_text(str(selection.get("context") or ""))
    ...
    title_shot = first_existing([first_shot, core_shot, outcome_shot])
    render_title_card(root, listing, notes, drafts, story_sections, hero_shot=title_shot)
    ...
    produced.append("verdict-card.png")
