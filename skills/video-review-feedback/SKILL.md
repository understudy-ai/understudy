...
- Real-edit gate:
  - if `post/capcut-edit-note.md` says the video came from a fallback preview renderer, shell compositor, missing-real-export recovery, or any other non-CapCut contingency path the user did not explicitly approve, do not treat that video as publish-ready
  - in that case, the review should explicitly call for a true CapCut re-edit rather than praising the preview render for being merely reviewable
- Auto-repair rule:
  ...
  - do not use shell preview renderers as the bounded repair path when the current requirement is a real CapCut edit; the repair loop should stay on the real edit route
  - if `post/final-video.mp4` is missing or the only available MP4 is a known preview/fallback stand-in, do not synthesize a new fake-final video just to keep the pipeline moving; record the blocked truth and send the run back to a real CapCut export
...
- `post/video-plan.md`
- `post/capcut-edit-note.md` if it exists
...
6. If the current MP4 is only a fallback preview render rather than a real CapCut export, do not grade it as a near-final cut. Treat the next move as a true CapCut re-edit.
7. If the verdict is clearly fixable in one bounded pass, do one repair loop:
   - `revisionMode: "edit"` for framing, pacing, text, packaging, CapCut-export quality
...
3. read 2-3 representative frames such as:
   ...
   - the scorecard / verdict frame
4. compare those visuals against:
   - `post/video-plan.md`
   - `experience/review-brief.md`
   - `experience/notes.json`
   - `post/capcut-edit-note.md` when it exists
...
- score / verdict beats that overwhelm the actual product proof
...
- A ready cut should usually satisfy all of these:
  ...
  - the product proof still dominates over score / verdict packaging
...
- A fallback preview render that stands in for a failed real CapCut edit should never be marked `ready` unless the user explicitly asked for a preview-only fallback.
...
- Prefer `revisionMode: "edit"` for preview-render substitutions that need a true CapCut re-edit.
...
- call out when the edit underuses available product motion or spends too much time on title / promise / score packaging
...
- prefer edit instructions like "shorten title card to 3.0s" or "replace the scorecard chip text" over broad advice like "make it snappier"
...
{
  "summary": "string",
  "sourceRoute": "local_capcut | web_capcut | fallback_preview | unknown",
