...
metadata:
  {
    "understudy": { "emoji": "🧩", "requires": { "anyBins": ["claude", "codex", "opencode", "pi"] } },
  }
---
...
**⚠️ CRITICAL: Never review PRs in Understudy's own project folder!**
...
8. **NEVER start Codex in ~/.understudy/** - keep it out of your live runtime state and session store.
9. **NEVER review or patch inside your live Understudy deployment/worktree** - clone to a temp dir or use a separate git worktree instead.
...
For long-running background tasks, rely on the parent agent to monitor `process action:log` and `process action:poll`.
Do not depend on external notification CLIs that are not part of this repository.

If you want a crisp completion marker, append a final print step to the child prompt:

```text
When completely finished, print exactly one final line in this format:
DONE: [brief summary]
```
...
When completely finished, print exactly:
DONE: Built todos REST API with CRUD endpoints'"
```

---

## Practical Notes (Jan 2026)
