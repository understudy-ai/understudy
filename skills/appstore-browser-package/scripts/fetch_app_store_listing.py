...
def normalize_review(review: dict[str, Any]) -> dict[str, Any]:
    normalized = {
        ...
        "contents": clean_text(review.get("contents")),
        "reviewerName": clean_text(review.get("reviewerName")),
        "rating": int_or_none(review.get("rating")),
    }
    return {key: value for key, value in normalized.items() if value not in {"", None}}


def extract_review_samples(page_html: str, limit: int = 5) -> list[dict[str, Any]]:
    marker = '"review":'
    ...
    while len(samples) < limit:
        start = page_html.find(marker, cursor)
        if start < 0:
            break
        payload_start = start + len(marker)
        try:
            parsed, offset = decoder.raw_decode(page_html[payload_start:])
        except json.JSONDecodeError:
        ...
        if not isinstance(parsed, dict):
            continue
        sample = normalize_review(parsed)
        sample_id = clean_text(sample.get("id"))
        if not sample or (sample_id and sample_id in seen_ids):
            continue
        if sample_id:
...
def build_listing(page_html: str, url: str) -> dict[str, Any]:
    ...
    review_count = int_or_none(first_non_empty(aggregate.get("reviewCount"), regex_search(page_html, [r'"reviewCount":"?([0-9,]+)"?'])))
    ...
    review_samples = extract_review_samples(page_html)
