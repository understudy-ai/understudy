...
23. Prefer at least one short live iPhone clip of the core loop or payoff when the task is dynamic enough to benefit from motion. A strong video-stage cut should not be forced to rely on still screenshots alone when a safe clip was reachable.
...
25. Do not stop with a "looks nice" package. Before leaving Stage 4, ask whether the current evidence could cut into a genuinely watchable English-first walkthrough with a beginning, middle, and end. Most good runs should land around `60-120 seconds`; only push longer when the evidence clearly earns it.
...
29. If one more short clip or a fifth screenshot was safely reachable and would clearly deepen the final edit, prefer capturing it now over leaving the video stage to compensate with extra packaging.
...
42. Motion-proof rule for depth: if the strongest truthful beat involves visible typing, processing, reordering, or a changed-state transition and you still leave without any short live clip, assume the video stage will look flatter and write that limitation explicitly instead of claiming the run was strong.
...
45. For creator and editor apps, do not stop at "tool opened". A strong run should usually prove at least: import/open -> one visible edit -> one result or saved-state surface -> one trust/detail/undo/history/export beat.
46. Deliberately capture one thumbnail-grade frame where the app result or friction is large, readable, and emotionally clear. Do not leave Stage 4 with only browser detail pages or tiny mid-gesture frames as the best visual.
47. A deeper review should usually answer six viewer questions, not four: what is it for, how fast does value start, what changed when I tried the main thing, what persisted or exported, what subtle detail surprised me, and what kind of user should skip it.
48. If you found a hidden detail, trust signal, persistence proof, or non-obvious limitation, record it explicitly in both markdown handoff files and the structured notes instead of burying it in a generic paragraph.
...
49. Prefer `10-18` distinct screenshots when the app safely supports it, not just the minimum package. If the current evidence still feels thin, keep pushing for one extra revisit, settings, trust, limit, or hidden-detail beat before leaving Stage 4.
50. If you discover a useful detail that was absent from the listing, capture both the surface itself and one nearby frame that shows why it matters in practice, so the video stage can narrate it like a human insight instead of a generic feature list.
...
53. Candidate-pool rule: around the strongest proof beats, deliberately capture 2-3 nearby variants when the surface changes quickly or the framing might matter later. The editor should have a real choice set for thumbnail, opening frame, outcome frame, and closing frame.
54. Shell restraint rule: outside bounded screenshot/clip capture and final artifact writing, do not use `bash` to reason about the live app state. If the next decision depends on what is on screen, prefer `gui_observe`.
54. Account-wall stop rule: if a sign-in, sign-up, phone/email verification, or social-login chooser blocks the first meaningful working surface of the app, treat that as the real Stage 4 story instead of grinding through login options. Capture the blocker clearly, write an honest `partial` handoff, and move on.
55. Creator-app blocker rule: for creator tools, if you never reach a real editor, canvas, template picker, or importable project surface before login/account creation is required, stop the exploration after collecting the required blocker evidence. Do not keep drilling through alternate login methods.
56. Account-wall proof rule: if the first usable flow turns into `Continue to sign up`, `Continue another way`, `Continue with email`, `Continue with phone number or email`, `Sign in`, `Log in`, or a similar account-first screen, treat that as the primary friction proof set. Capture the cleanest pre-login surfaces, record that the editor could not be reached without account flow, write the notes immediately, and continue the pipeline.
57. Auth-stop rule: once a sign-up wall is visible, you may collect at most one deeper account-proof screen. Do not tap through multiple auth variants. Pick one branch only, capture the blocker, and stop.
58. Auth-input rule: if the visible surface contains a login input such as `Phone number or email`, `Email`, `Password`, `Continue with email`, or another editable auth field, that is already deep enough proof. Do not focus the field, do not type, and do not try to submit credentials. Capture the screen as the blocker evidence and stop Stage 4.
59. Auth backout rule: if you used one allowed deeper auth branch and then fail to back out cleanly in one bounded attempt, stop immediately instead of exploring more auth UI. A failed back-arrow recovery is itself proof that the run is inside the account wall.
60. Account-picker rule: if the first surface is a country-code picker, phone-region chooser, country list sheet, or another auth-adjacent picker with a search field, treat that sheet itself as auth-wall evidence. Capture it, optionally try one obvious dismiss action only if it is visually trivial, and then stop if the app remains in auth flow. Do not keep dragging, escaping, or probing the sheet repeatedly.
61. Platform-native-login priority rule: if login is unavoidable and the app offers the platform-native sign-in route, prefer the option that minimizes credential typing and account sprawl unless that route is visibly broken or unavailable.
...
The exploration must be deep enough to fill a strong proof-first short. To achieve this
...
11. **Discover something non-obvious**: explore a hidden feature, trust surface, pricing surface, saved-state detail, or another in-app limit.
...
14. Capture `09-Trust-Or-Limit.png` and `10-Revisit.png`.
...
09-Trust-Or-Limit.png        — settings, pricing, trust, or limitation surface
...
Default evidence target: **12-18 screenshots**, **2-3 clips**, **3 completed scenarios**.

Before stopping, always run:

```bash
python3 skills/app-explore/scripts/build_evidence_catalog.py "$ROOT_DIR"
```

This writes `experience/evidence-catalog.json` and enforces the default evidence floor unless the run truthfully stayed partial because of an auth wall, permission block, or similarly honest constraint.
...
5. Default screenshot package:
   ...
   - optional `08-Hidden-Detail-Or-Pricing.png`
6. Write:
...
Keep that plan coherent. The goal is not to tap everything. The goal is to return with enough linked evidence for the video stage to make a convincing review.
...
This keeps the video stage honest when the exploration stayed partial.
...
- Persist that `{x,y,width,height}` tuple mentally and reuse it for every later screenshot in this stage.
...
- Prefer `python3 skills/iphone-mirroring-basics/scripts/capture_mirroring_screenshot.py <output_path>` for final screenshot artifacts. It manages a unique temp file, prefers direct `iPhone Mirroring` window capture by window ID, and avoids fragile hand-written `mktemp ... XXXX.png` flows.
...
Account-wall handling details:

- One alternate path is enough to verify the blocker. Do not tap through more than one extra login variant after the primary sign-up screen.
- If `Sign in with Apple` / `Continue with Apple` is visible, treat that as the preferred single alternate path. Do not choose `phone number or email` first when the Apple path is already offered.
- For creator apps blocked by account-first onboarding, a clean proof ladder is usually:
  - `01-First-Screen`: first launch or first gate
  - `02-Main-Screen`: primary sign-up chooser
  - `03-Core-Task`: either the first sign-up chooser itself or one single alternate path, preferably `Continue with Apple` when available
  - `04-Outcome-Or-Friction`: the phone/email requirement or equivalent account wall
- If the app opens directly into a country-code picker or phone-entry sheet, that can replace the normal outer chooser ladder. In that case:
  - `01-First-Screen`: country picker / phone-entry sheet
  - `02-Main-Screen`: outer auth surface only if it becomes visible after one trivial dismiss
  - `03-Core-Task`: preferred Apple-login branch if it becomes visible without extra auth drilling; otherwise optional single auth-adjacent proof beat that still avoids typing
  - `04-Outcome-Or-Friction`: the clearest phone/email or account requirement screen
- Do not use both `Continue another way` and `Continue with email` in the same run. Pick exactly one deeper branch. If the first wall already shows email or phone requirement clearly enough, do not tap further.
- If both Apple and phone/email branches are visible, prefer Apple and do not tap the phone/email field first.
- Once an auth text field or placeholder like `Phone number or email` is visible, the ladder is complete. Capture it as `04-Outcome-Or-Friction` and jump straight to Step 6.
- Do not use more than one dismiss attempt on a country picker or phone-entry sheet. After one obvious close / grabber / escape attempt, either capture the outer wall if it appears or stop and write the blocker notes.
- Once that ladder exists, jump straight to Step 6 and mark the run `partial`.

## Step 5: Explore deeply — cover the app's real feature surface

The goal is a thorough walkthrough that a viewer would find genuinely informative, not a smoke test. The evidence must support a real proof-first video with enough variety that the editor is choosing among receipts, not stretching one weak frame.
...
- `09-Trust-Or-Limit.png` — settings, privacy, account, pricing, or limitation surface
...
Capture every screenshot with the **same consistent method**: use the latest `capture_rect` from the GUI tool, capture the real `iPhone Mirroring` window through the helper in `iphone-mirroring-basics`, then crop with the standard proportional insets from `iphone-mirroring-basics`. Verify each saved PNG has consistent dimensions within a small tolerance so the video stage can cut them together cleanly.
...
- Creator / photo / video / design apps:
  ...
  - if an account wall appears before any real editor/template/import surface, stop treating the app like a normal creator flow; capture the login wall as the honest blocker and finalize notes instead of bouncing between login options
  - an auth input field counts as the final blocker surface; do not tap inside it just to prove the wall is real
  - a country picker / phone-region sheet is already part of the auth wall; do not spend multiple actions trying to escape it
  - when a creator app exposes Apple auth alongside other providers, prefer Apple as the single deeper auth proof path
...
- Planner / calendar / scheduling / reminder apps:
  ...
  - if that create -> saved -> revisit ladder is visually clear, prefer capturing it as the primary short live clip rather than leaving the video stage with only still screenshots
...
- Keep the secondary proof surface inside the app whenever possible. Do not treat generic iOS Settings, permission settings, or a stray system panel as valid depth unless the product's core value is literally about system settings or permissions.
- If an intended in-app action unexpectedly jumps to iOS Settings or another unrelated system surface, record it as a misroute or friction beat, back out once, and continue from the last known in-app boundary. Do not keep exploring the Settings tree just to prove "depth."
...
- Publication-readiness check: before leaving the app, mentally ask whether the video stage could make a strong English-first proof-first walkthrough with mostly product proof and only light packaging. If the answer is no, push for one more result, revisit, motion, hidden-detail, or limit beat when safe.
...
- Prefer the direct `iPhone Mirroring` clip helper so the proof beat stays attached to the phone window itself instead of the whole desktop.
- Practical shell path for a short motion beat is:
...
CLIP_LOG="$ROOT_DIR/experience/clip-recording.log"
nohup python3 skills/iphone-mirroring-basics/scripts/capture_mirroring_clip.py "$CLIP_PATH" --duration "$CLIP_DURATION" >"$CLIP_LOG" 2>&1 &
sleep 1
```

- **Always** set `CLIP_DURATION` to the appropriate length (10-25s) before recording. Do not copy the example value verbatim.
...
- The clip helper already crops away the iPhone Mirroring chrome. Do not replace it with whole-display capture unless the helper fails.
...
- If the first clip only shows setup and not payoff, try to capture a second short clip for the strongest visible result, follow-up, or revisit beat so the video stage is not forced into three static proof cards in a row.
- Before ending Stage 4 without any clip, explicitly ask whether the final video would now feel too static or too card-led. If yes and one safe motion beat is still reachable, capture it now rather than leaving the video stage to fake energy with extra text.
...
2. Capture through the helper: `python3 skills/iphone-mirroring-basics/scripts/capture_mirroring_screenshot.py "$TARGET_PATH"`
3. Do not mix in ad hoc full-screen captures, manually cropped desktop screenshots, or gui-observe image exports.
...
1. **After capturing screenshot 04** (primary task result): write a first draft of
   ...
   For account-wall runs, this is not optional: once the blocker ladder exists,
   write the notes immediately and end the exploration instead of probing more
   login paths.
...
- [ ] At least 12 screenshots in `experience/screenshots/` unless the app blocked early
...
- [ ] No unresolved stray system-surface branch is being mistaken for product depth

Mark `deep` when all are met. Mark `partial` when 3-4 are met. Mark `shallow` only when genuinely blocked.
If the app is blocked by an account wall before any meaningful working surface,
the blocker ladder plus structured notes overrides the usual depth gate: `01-04`
and honest notes are enough, and you should stop there.

## Step 6: Finalize the structured review notes

Also write `experience/review-brief.md` as a human-readable handoff for the video stage.
...
```json
{
  ...
  "scriptHooks": {
    "titleCandidates": [
      "I tried <AppName> on iPhone",
      "<AppName> after one real test",
      "The honest first-run verdict on <AppName>"
    ],
    "openingHook": "string",
    "tensionLine": "string",
    "payoffLine": "string",
...
- `hiddenDetails` should capture the 1-3 less obvious truths you only learned by actually using the app.
- `limitations` should capture the most honest reasons a viewer might bounce even if the app is good.
- `titleCandidates` should usually contain at least 3 distinct, viewer-facing options rather than one placeholder.
- `narrationLines` should usually be `8-14` lines totaling `140-280 spoken words` (roughly `60-120 seconds` at natural pace). Stretch toward `120-150 seconds` only when the proof set is unusually rich. Do not pad a thin run just to hit a long target.
...
- `evidenceMoments` should usually name the exact screenshot or surface that proves each claim, so the video stage can lift them without guessing.
- `thumbnailCandidates` should prefer in-app proof frames such as the core task, saved result, export result, or strongest visible friction moment. Browser screenshots should only appear here as a fallback.
...
- `primaryLoop`, `secondaryProof`, and `fallbackStory` should stay simple and concrete so the video stage can turn them into cards without guessing.
...
- `Best For` and `Avoid If` should read like review advice that the video stage can almost quote directly.
- The `Must Show In Video` section should call out specific screenshots or moments that the video stage can translate into cards and narration.
...
- `Video Angle` should be the one-sentence thesis the video stage should lean on.
...
- If the run already has `01-04` plus one clip or a strong revisit frame, assume the video stage would rather receive honest partial notes now than a longer but messier unclosed exploration.
- If the app is blocked by login/account creation and you already have a clear `01-04` blocker ladder, stop exploring immediately and write the handoff files. The truthful blocker is more valuable than a longer unclosed run.
