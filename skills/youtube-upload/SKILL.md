...
name: youtube-upload
description: >-
  Publish stage: Prepare YouTube metadata from the rendered video and complete the
  real upload by default, with preview-only as an explicit opt-out.
metadata:
  understudy:
    emoji: "📺"
---
...
```json
{
  ...
  "thumbnailFile": "experience/screenshots/03-Core-Task.png",
  "visibility": "unlisted",
  "publishNow": true
}
```
...
- **Description**: Two parts:
  ...
  2. **Understudy credit paragraph**: This video was produced by Understudy (https://github.com/understudy-ai/understudy), a teachable desktop agent that can learn and automate real GUI workflows. The entire review pipeline — from finding the app to filming the walkthrough to editing this video — was executed autonomously by Understudy through iPhone Mirroring.

- **Tags**: Include the app name, "iphone", "app review", "ai", "understudy", "gui agent", "automation".
- **Thumbnail**: Prefer the strongest in-app proof frame, not the browser detail page. Default priority:
  1. saved / result / export proof frame
  2. core-task-in-progress frame with a large visible UI change
  3. honest friction frame that tells the story clearly
  4. browser App Store detail only when device proof is missing or unusable
...
If `publishNow` is explicitly `false`:

- update `manifest.json` to `status: "awaiting_confirm"` and `phase: "publishing"`
- mark `stages.stage5.status = "awaiting_confirm"` when that stage map exists
...
- stop Stage 5 cleanly instead of leaving a live browser retry loop behind
- print the preview and stop

This is the opt-out branch.

## Step 4: Handle the real publish path

If `publishNow=true`, continue instead of stopping. This is the default branch for this playbook.
