...
user-invocable: true
metadata:
  { "understudy": { "requires": { "bins": ["curl", "git"] }, "primaryEnv": "GH_TOKEN" } }
---
...
IMPORTANT — No `gh` CLI dependency. This skill uses curl + the GitHub REST API exclusively. When configured, Understudy should inject `GH_TOKEN` into the skill environment. Pass it as a Bearer token in all API calls:
...
If empty, stop and tell the user to provide a token. Understudy should expose `GH_TOKEN` directly or via the skill configuration.
...
- If curl returns an HTTP 401 or 403 → stop and tell the user:
  > "GitHub authentication failed. Please check your `GH_TOKEN` configuration in Understudy and retry."
...
7. **Check claim-based in-progress tracking:**
   ...
   ```
   UNDERSTUDY_STATE_DIR="${UNDERSTUDY_HOME:-$HOME/.understudy}/gh-issues"
   CLAIMS_FILE="$UNDERSTUDY_STATE_DIR/claims.json"
   if [ ! -f "$CLAIMS_FILE" ]; then
     mkdir -p "$UNDERSTUDY_STATE_DIR"
     echo '{}' > "$CLAIMS_FILE"
   fi
   ```
...
- **Sequential cursor tracking:** Use a cursor file to track which issue to process next:
  ...
  ```
  UNDERSTUDY_STATE_DIR="${UNDERSTUDY_HOME:-$HOME/.understudy}/gh-issues"
  CURSOR_FILE="$UNDERSTUDY_STATE_DIR/cursor-{SOURCE_REPO_SLUG}.json"
  # SOURCE_REPO_SLUG = owner-repo with slashes replaced by hyphens (e.g., nicepkg-understudy)
  ```
...
- {SOURCE_REPO} — source repo where the issue lives
...
First, ensure GH_TOKEN is set. Check: `echo $GH_TOKEN`. If empty, stop and report that the token is missing; do not attempt interactive auth inside the sub-agent.
...
Use the `message_send` tool with:

- action: "send"
- channel: "telegram"
- recipient: "{notify_channel}"
- text: "✅ PR Created: {SOURCE_REPO}#{number}
...
If `--notify-channel` was provided, send the final summary to that Telegram channel using the `message_send` tool:
