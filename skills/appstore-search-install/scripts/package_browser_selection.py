...
def should_promote_alias(query: str, alias: str) -> bool:
    ...
    return (
        len(query_tokens) == 1
        and 2 <= len(alias_tokens) <= 3
        and alias_tokens[0].lower() == query_tokens[0].lower()
    )
...
def main() -> int:
    ...
    selection_source = clean(args.selection_source) or clean(listing.get("selectionSource")) or "editorial"
    ...
    winner = {
        ...
        "deviceSearchQuery": refined_device_search_query(
    ...
    selected_app = {
        ...
        "selectionSource": selection_source,
        "deviceSearchQuery": clean(args.device_search_query),
        "expectedCoreTask": clean(args.expected_core_task),
        "installed": False,
    }
