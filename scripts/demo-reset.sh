...
def terminal_status_contains(allowed: set[str], value) -> bool:
...
if episodes_dir.is_dir():
    for ep_dir in sorted(episodes_dir.iterdir()):
        ...
        if not terminal_status_contains(episode_terminal, data.get('status')):
...
if runs_dir.is_dir():
    for run_dir in sorted(runs_dir.iterdir()):
        ...
        if not terminal_status_contains(run_terminal, status):
            data['status'] = 'cancelled'
            ...
            for child in data.get('childSessions') or []:
                if not terminal_status_contains(child_terminal, child.get('status')):
