...
export function buildSkillsSection(skills?: Skill[]): string[] {
	...
	return [
		...
		"- If the selected artifact is labeled `Playbook`, treat it as a staged orchestration contract. After reading it, start executing the staged work immediately, typically by spawning child sessions when the artifact says to.",
		"- If the selected artifact is labeled `Worker`, treat it as bounded delegated work. Act on the contract immediately instead of turning it into a plan-only reply.",
		"- If none clearly apply: do not read any SKILL.md.",
		"Constraints: never read more than one skill up front; only read after selecting.",
		"- When the selected skill or artifact describes executable work and the needed tools are available, do the work now. Do not end the run with a plan-only reply such as 'I'll do that' or 'First I'll check'.",
		"- For `Playbook` and `Worker` artifacts, setup-only steps are not completion. Reading the artifact, creating folders, writing a starter manifest, or announcing intended stages are intermediate work only.",
		"- Keep executing within the same run until the requested outputs actually exist, the artifact reaches an explicit approval/input gate, or you hit a concrete blocker you can describe with evidence.",
		"- Do not stop after scaffolding and reply with progress-only text such as 'I started the pipeline' or 'I created the workspace' when the artifact is still executable.",
...
export function buildReasoningFormatSection(): string[] {
	return [
		...
		[
			...
			"If tools are required to complete the request, call them immediately instead of spending a reply on intention-only text.",
			"Do not end a run with a plan-only <final> such as 'I'll do that', 'Let me check', or 'First I'll look around' when the task is already executable.",
			"Do not use <final> for progress-only updates like 'I started', 'I’m working on it', or 'The pipeline is underway' unless the user explicitly asked for a progress ping; keep working instead.",
			"A truthful <final> should normally mean one of: the requested work is complete, a concrete blocker was encountered, or a real approval/input gate has been reached.",
			"Example:",
