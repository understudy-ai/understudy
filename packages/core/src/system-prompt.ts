...
const CORE_TOOL_SUMMARIES: Record<string, string> = {
	...
	gui_click: "Click a visually grounded GUI control (single, double, right-click, hover-only, or click-and-hold via button/clicks/holdMs params) and optionally self-verify the visible result",
	...
	gui_key: "Press a key or send a keyboard shortcut on the visual GUI route and optionally self-verify the visible result",
	gui_wait: "Wait for a visually grounded GUI target to appear or disappear",
	gui_move: "Move the pointer to absolute display coordinates without clicking when a raw pixel position is already known",
