	...
	it("records a real teach demo through /teach start and /teach stop", async () => {
		...
		try {
			...
			const runtime = createGatewaySessionRuntime({
				...
				videoTeachAnalyzer: {
					analyze: vi.fn(async () => ({
						title: "Teach the real Finder/TextEdit switch flow",
						objective: "Capture the Finder/TextEdit switch flow from the real desktop recording.",
						taskKind: "fixed_demo",
						parameterSlots: [],
						successCriteria: ["The flow can be replayed."],
						openQuestions: [],
						taskCard: {
							goal: "Capture the Finder/TextEdit switch flow from the real desktop recording.",
							scope: "Finder and TextEdit app switching smoke flow.",
							loopOver: "The current recorded interaction.",
							inputs: [],
							extract: ["App activation changes"],
							output: "A teach draft generated from the real recording.",
						},
						procedure: [],
						replayPreconditions: [],
						resetSignals: [],
						...
						provider: "smoke-test",
						model: "smoke-test",
						sourceLabel: "teach-real-smoke.mov",
						analysisMode: "event_guided_evidence_pack",
						episodeCount: 1,
						keyframeCount: 1,
						eventCount: 1,
						evidenceSummary: "Smoke summary from real teach recording.",
						durationMs: 1_000,
						keyframes: [],
					})),
				} as any,
