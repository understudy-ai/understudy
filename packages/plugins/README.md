...
- **Registry**: `UnderstudyPluginRegistry` manages loaded plugins and collects their contributions (tools, gateway methods, channel factories, CLI commands)
...
- **Plugin API**: plugins receive an `UnderstudyPluginApi` handle to register tools, gateway methods, channel factories, and CLI sub-commands
...
| `UnderstudyPluginApi` | type | API surface passed to `register()` — `registerTool`, `registerGatewayMethod`, `registerChannelFactory`, `registerCli` |
