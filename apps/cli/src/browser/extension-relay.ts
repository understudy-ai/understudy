...
import { isLoopbackAddress } from "@understudy/gateway";
import {
...
function rawDataToString(data: WebSocket.RawData, encoding: BufferEncoding = "utf8"): string {
...
function getRelayAuthTokenFromRequest(req: IncomingMessage, url?: URL): string | undefined {
  const headerToken = getHeader(req, UNDERSTUDY_EXTENSION_RELAY_HEADER)?.trim();
  if (headerToken) {
    return headerToken;
  }
...
export type UnderstudyChromeExtensionRelayServer = {
  host: string;
  port: number;
  baseUrl: string;
  cdpWsUrl: string;
...
type RelayRuntime = {
  server: UnderstudyChromeExtensionRelayServer;
...
const relayInitByPort = new Map<number, Promise<UnderstudyChromeExtensionRelayServer>>();
...
export function getUnderstudyChromeExtensionRelayRuntimeHeaders(url: string): Record<string, string> {
  ...
  return { [UNDERSTUDY_EXTENSION_RELAY_HEADER]: token };
}

export async function ensureUnderstudyChromeExtensionRelayServer(opts: {
  cdpUrl: string;
  gatewayToken?: string;
}): Promise<UnderstudyChromeExtensionRelayServer> {
  ...
  const extensionReconnectGraceMs = envMsOrDefault(
    "UNDERSTUDY_EXTENSION_RELAY_RECONNECT_GRACE_MS",
    DEFAULT_EXTENSION_RECONNECT_GRACE_MS,
  );
  const extensionCommandReconnectWaitMs = envMsOrDefault(
    "UNDERSTUDY_EXTENSION_RELAY_COMMAND_RECONNECT_WAIT_MS",
    DEFAULT_EXTENSION_COMMAND_RECONNECT_WAIT_MS,
  );

  const initPromise = (async (): Promise<UnderstudyChromeExtensionRelayServer> => {
    const relayAuthToken = await resolveUnderstudyRelayAuthTokenForPort(info.port, {
      gatewayToken: opts.gatewayToken,
    });
    const relayAuthTokens = new Set(await resolveUnderstudyRelayAcceptedTokensForPort(info.port, {
      gatewayToken: opts.gatewayToken,
    }));
    ...
    const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
      switch (cmd.method) {
        case "Browser.getVersion":
          return {
            protocolVersion: "1.3",
            product: "Chrome/Understudy-Extension-Relay",
            revision: "0",
            userAgent: "Understudy-Extension-Relay",
            jsVersion: "V8",
          };
    ...
    const server = createServer((req, res) => {
      ...
      if (req.method === "OPTIONS") {
        ...
        const allowedHeaders = new Set(["content-type", UNDERSTUDY_EXTENSION_RELAY_HEADER, ...requestedHeaders]);
        res.writeHead(204, {
      ...
      if (
        ...
        const payload: Record<string, unknown> = {
          Browser: UNDERSTUDY_EXTENSION_RELAY_BROWSER,
    ...
    server.on("upgrade", (req, socket, head) => {
      ...
      if (pathname === "/extension") {
        ...
        wssExtension.handleUpgrade(req, socket, head, (ws: WebSocket) => {
      ...
      if (pathname === "/cdp") {
        ...
        wssCdp.handleUpgrade(req, socket, head, (ws: WebSocket) => {
    ...
    try {
      ...
      if (
        ...
        (await probeAuthenticatedUnderstudyRelay({
          baseUrl: info.baseUrl,
        }))
      ) {
        const existingRelay: UnderstudyChromeExtensionRelayServer = {
          host: info.host,
          port: info.port,
          baseUrl: info.baseUrl,
    ...
    const relay: UnderstudyChromeExtensionRelayServer = {
      host,
      port,
      baseUrl,
...
export async function stopUnderstudyChromeExtensionRelayServer(opts: { cdpUrl: string }): Promise<boolean> {
  const info = parseBaseUrl(opts.cdpUrl);
  const existing = relayRuntimeByPort.get(info.port);
  if (!existing) {
    return false;
