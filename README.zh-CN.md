...
Understudy 主要在 **macOS** 上开发和验证，核心界面（CLI、Gateway、浏览器、消息频道）在设计上都是跨平台的。

- **macOS：** 提供完整的 GUI runtime、截图 grounding 和演示教学能力
- **Windows：** 已提供基于 PowerShell 的 GUI MVP，支持 `gui_observe`、`gui_wait`、`gui_key`、`gui_type`、`gui_click`、`gui_drag`、`gui_scroll`、`gui_move`
- **Linux：** GUI 后端仍在规划中

当前 Windows 后端主要覆盖截图和输入分发；演示录制以及按窗口索引选择目标窗口还没有实现。
...
- **GUI 后端** — Linux (AT-SPI) 支持，以及 Windows 侧更深入的验证与加固
- **技能** — 热门应用和工作流的新技能模块
