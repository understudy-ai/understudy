...
**真正独特的是它之后的学习回路。** 你可以通过演示教学，让重复的日常工作自动 crystallize 成可复用 skill，并逐步升级到更快的执行路线。

- **先是通用 Agent**：同一个 runtime 里统一调度 GUI、browser、shell、web、files、memory、messaging、schedule 和 subagents。
- **然后是 Computer Use**：具备 Cowork 这一类产品的桌面操作能力，但保持本地优先、开源和自带模型/API key。
- **最后是 Teach + Crystallize + Route Upgrade**：显式教学、日常自动结晶化和路线升级放在同一套系统里。
...
这是 Computer Use 的实际演示：从 Telegram、Discord、Slack 或 8 个渠道中的任一个发送消息。Understudy 在你的桌面上接收、执行多步骤任务 —— 浏览器搜索、GUI 自动化、文件创建、Shell 命令 —— 并将结果回复。即使你不在电脑前。
