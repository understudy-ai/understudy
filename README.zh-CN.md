...
| 路线 | 实现方式 | 覆盖范围 |
|------|---------|---------|
| **GUI** | 13 个工具 + 截图 grounding + 原生输入 | 任意 macOS 桌面应用 |
| **浏览器** | Playwright 托管 + Chrome extension relay | 任意网站，保持登录态 |
...
**状态：** 部分实现。路线偏好、route guard 策略和 teach 路线标注已经在工作。完全自动的路线发现和提升还在开发中。
...
- Route guard 追踪每条路线的连续失败次数。如果某条路线反复失败（比如 GUI grounding 对某个应用不可靠），会提示 agent 切换到替代路线。
- Teach 产出的 skill 里每步标注了 `preferred` / `fallback` / `observed` 路线，agent 可以跳过 GUI 直接用更快路径。
- 浏览器模式自动降级：先试 Chrome extension relay，失败则切换到托管 Playwright。
...
| tesseract | `brew install tesseract` | `vision_read` 本地 OCR。不安装则自动跳过 OCR |
...
**启用功能：** 鼠标点击、输入、拖拽、滚动、针对应用目标的快捷键，以及演示事件捕获。

**缺失时：** 输入驱动类 GUI 工具会被阻止；如果屏幕录制权限可用，截图和 GUI read 这类观察型能力仍可能可用。
...
**启用功能：** GUI grounding 截图与验证、截图式 GUI 读取，以及演示视频录制。

**缺失时：** 基于截图 grounding 的 GUI 工具会被阻止；`gui_keypress`、`gui_hotkey` 这类纯键盘工具仍可能可用，部分工具也还能以不带 `target` 的方式运行。
...
>
> 如果授权后 GUI 能力仍然不完整，运行 `understudy doctor --deep` 检查 native helper、grounding、OCR、浏览器运行时和 teach-analysis 依赖是否齐全。
